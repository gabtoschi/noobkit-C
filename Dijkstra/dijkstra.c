...
int *dijkstra(MGraph *g, int start){
	...
	while (!heapEmpty(Q)){
		...
		for (i = 0; adj[i] != NoEdge; i++){
			// caso ele nao esteja no caminho e o custo da ligacao entre os dois seja a menor
			if (!addCheck[adj[i]] && getEdgeCostM(g, v.vertex, adj[i]) + v.cost < heapGetCost(Q, adj[i])){
				// atualize sua prioridade
				heapUpdate(Q, adj[i], getEdgeCostM(g, v.vertex, adj[i]) + v.cost);
